...
void GraphicsEngine::drawLines(const RotRect & rot, int count) {
	SDL_Point point[] = {
		{rot.TL.x, rot.TL.y},
		{rot.TR.x, rot.TR.y},
		{rot.BL.x, rot.BL.y},
	...
	if (SDL_RenderDrawLines(renderer, point, count) != 0) {
		std::cout << "hello" << SDL_GetError() << std::endl;
	}
}

void GraphicsEngine::drawLines(const Point2& TL, const Point2& TR, const Point2& BL, const Point2& BR, int count) {
	SDL_Point point[] = { 
		{TL.x, TL.y},
		{TR.x, TR.y},
		{BL.x, BL.y},
